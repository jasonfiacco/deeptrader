...
class TradingEnvironment(gym.Env, TimeIndexed):
    ...
    def __init__(self,
        ...
        self._observation_lows = kwargs.get('observation_lows', np.finfo(np.float32).min)
        self._observation_highs = kwargs.get('observation_highs', np.finfo(np.float32).max)
